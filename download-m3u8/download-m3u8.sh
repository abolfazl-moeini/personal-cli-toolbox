...
# Updated to handle master playlists by prompting the user to select a stream.
...
download_stream() {
    ...
    aria2c --console-log-level=warn --user-agent="$USER_AGENT" -x"$CONNECTIONS" -d"$TEMP_DIR" -i"$TEMP_DIR/urllist.txt"
...
main() {
    ...
    if echo "$MASTER_PLAYLIST_CONTENT" | grep -q "#EXT-X-STREAM-INF"; then
        ...
        # Use awk to parse stream info and the URL on the next line
        ...
        local CHOSEN_URL
        # Check if the playlist path is a full URL or relative
